...
def bday(): # file reading function
    bF=open('bday.txt', 'r')
    print(bF.read())
    bDay = bF.read()
    bF.close()
...
'''
onOff = input("Do you want to turn on the alarm? y/n: ").lower() # initiate the loop

if onOff == 'n': # NO ALARM
    print("Alarm is off. Goodbye!")

bDay = input("Enter your birthday (MM-DD) or 1 for save: ")

if bDay != '1': # save and write the birthday 
    bF =open('bday.txt', 'w')
    bF.write(bDay)
    bF.close()
else:
    bday() # read already written birthday
